...
const ChatInterface = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: "1", sender: "user", text: "Hi, I'm interested in the hotel management system.", time: "10:15 AM" },
    { id: "2", sender: "bot", text: "Hello! We'd love to help you with that. Are you looking for a cloud-based solution or on-premise?", time: "10:16 AM" },
    { id: "3", sender: "user", text: "Cloud-based would be better for our team. We have 5 locations currently.", time: "10:20 AM" },
    { id: "4", sender: "bot", text: "Perfect. Jordan can handle multi-location synchronization in real-time. Would you like a demo of the central dashboard?", time: "10:22 AM" },
  ]);
        ...
        <div className="bg-white/90 backdrop-blur-2xl rounded-[32px] border border-white p-4 shadow-xl flex flex-col h-full overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-1.5 pb-28">
            {[
              ...
              { id: 3, name: "Alex Thompson", text: "Just looking around...", time: "Yesterday" },
              { id: 4, name: "Marcus Wright", text: "How do I integrate API?", time: "Yesterday" },
              { id: 5, name: "Elena Rodriguez", text: "Let's schedule a demo.", time: "Tuesday" },
              { id: 6, name: "James Wilson", text: "I need support...", time: "Monday" },
              { id: 7, name: "Linda Moore", text: "Can we talk later?", time: "Sunday" },
            ].map((customer) => (
              <button 
                key={customer.id} 
          ...
          <div className="bg-white/90 backdrop-blur-2xl rounded-[32px] border border-white p-5 shadow-xl flex flex-col h-full overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar pb-32">
              <div className="flex flex-col items-center mb-6 text-center">
                ...
                <h3 className="text-lg font-black text-gray-900 tracking-tight">Sarah Jenkins</h3>
              </div>

              {/* Tightened Media Sections */}
              <div className="space-y-2 mb-6">
                {[
                  { label: 'Documents', count: '128 files, 200mb', icon: <FileText size={16} />, border: 'border-purple-100', bg: 'bg-purple-50', text: 'text-purple-600' },
                  { label: 'Photos', count: '128 files, 200mb', icon: <ImageIcon size={16} />, border: 'border-orange-100', bg: 'bg-orange-50', text: 'text-orange-600' },
                  { label: 'Other', count: '128 files, 200mb', icon: <Info size={16} />, border: 'border-cyan-100', bg: 'bg-cyan-50', text: 'text-cyan-600' }
                ].map((media) => (
              ...
              <div className="mb-6 p-1">
                 ...
                 <div className="relative pt-4">
                    ...
                    <div className="h-3 w-full bg-gray-100 rounded-full flex overflow-hidden p-0.5 gap-0.5 shadow-inner">
                      <div className="h-full w-1/4 bg-gray-300 rounded-full" /> {/* Grey */}
                      <div className="h-full w-1/4 bg-blue-500 rounded-full shadow-[0_0_8px_rgba(59,130,246,0.4)]" /> {/* Blue */}
                      <div className="h-full w-1/4 bg-orange-400 rounded-full" /> {/* Orange */}
                      <div className="h-full w-1/4 bg-red-500 rounded-full" /> {/* Red */}
                    </div>
                 </div>
              </div>
              ...
              <div className="space-y-2">
                 <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-2">Progress Tracker</span>
                 {[1, 2, 3, 4].map((i) => (
                   <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-white border border-gray-50 hover:border-blue-100 cursor-pointer transition-all group shadow-sm">
                     <span className="text-[11px] font-bold text-gray-700">Milestone Todo</span>
